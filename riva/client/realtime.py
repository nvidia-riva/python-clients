...
class RealtimeClientTTS:
    ...
    async def _update_session(self):
        ...
        if hasattr(self.args, 'voice_name') and self.args.voice_name:
            self._safe_update_config(session_config, "voice_name", self.args.voice_name, "input_text_synthesis")
        ...
        if hasattr(self.args, 'num_channels') and self.args.num_channels:
            self._safe_update_config(session_config, "num_channels", self.args.num_channels, "output_audio_params")
            overrides.append("num_channels")

        if hasattr(self.args, 'audio_format') and self.args.audio_format:
            self._safe_update_config(session_config, "audio_format", self.args.audio_format, "output_audio_params")
        ...
        if (hasattr(self.args, 'zero_shot_transcript') and self.args.zero_shot_transcript) or \
           (hasattr(self.args, 'zero_shot_quality') and self.args.zero_shot_quality) or \
           (hasattr(self.args, 'zero_shot_audio_prompt') and self.args.zero_shot_audio_prompt):
            
            if "zero_shot_config" not in session_config:
                session_config["zero_shot_config"] = {}
            
            if hasattr(self.args, 'zero_shot_transcript') and self.args.zero_shot_transcript:
                self._safe_update_config(session_config["zero_shot_config"], "audio_prompt_transcript", self.args.zero_shot_transcript)
                overrides.append("zero_shot_transcript")

            if hasattr(self.args, 'zero_shot_quality') and self.args.zero_shot_quality:
                self._safe_update_config(session_config["zero_shot_config"], "prompt_quality", self.args.zero_shot_quality)
                overrides.append("zero_shot_quality")

            if hasattr(self.args, 'zero_shot_audio_prompt') and self.args.zero_shot_audio_prompt:
                try:
                    with open(self.args.zero_shot_audio_prompt, 'rb') as f:
                        audio_data = f.read()
                        self._safe_update_config(session_config["zero_shot_config"], "audio_prompt_bytes", base64.b64encode(audio_data).decode('utf-8'))
                        overrides.append("zero_shot_audio_prompt")
                except Exception as e:
                    logger.warning("Failed to load zero-shot audio prompt: %s", e)
